...
#contigs <- c("1270", "1270", "1270", "3357")
#contig.adjs <- c(42, 5830, 6544, 3712)
#contig.letters <- c("e", "f", "g", "h")
#xmins <- c(0, 630000, 750000, 540000)
#xmaxs <- c(100000, 710000, 850000, 660000)
...
png("candidate4x4_ng.png", width = 900, height = 720)
...
for (i in seq(1, length(contigs))) {
  ...
  png.plot(xlabel, ylabel, xlimit, ylimit, yaxis.textline)
  ...
  lines(dxy.rf$V1 - contig.adj, dxy.rf$V2,
	...
	if (i == 2) {
	   axis(1, at = c(0, 20000, 40000, 60000, 80000, 100000), labels = c("0", "20000", "40000", "60000", "80000", "100000"), cex.axis = 1.5)
	} else {
       	  png.xaxis()
  	}
  png.yaxis(yaxis.at, yaxis.at, yaxis.line)
  ...
  points(ffae$V2 - contig.adj, rep(ylimit[2] - 0.0065, length(ffae$V2)), pch = 20, col = fae.col)
  points(fsat$V2 - contig.adj, rep(ylimit[2] - 0.0085, length(fsat$V2)), pch = 20, col = sat.col)
  ...
  for (j in seq(1, length(subgenes$name))) {
    lines(c(subgenes$start[j], subgenes$end[j]), c(gene.ypos + subgenes$yadj[j], gene.ypos + subgenes$yadj[j]), lwd = lwidth, col = "grey25")
    text(subgenes$start[j] + ((subgenes$end[j] - subgenes$start[j]) / 2) + subgenes$xadj[j], genetext.ypos + subgenes$yadj[j], labels = subgenes$name[j], cex = 1.5)
  }

  if (i == 1) {
    arrows(575250, 0.03, 575250, 0.026, length = 0.08, col = "grey25", lwd = 3)
    arrows(612000, 0.03, 612000, 0.026, length = 0.08, col = "grey25", lwd = 3)
    arrows(642000, 0.03, 642000, 0.026, length = 0.08, col = "grey25", lwd = 3)
  }
